...
import com.xun.smalltoutiao.libcomm.base.CommBaseApi;
...
public class CommRetrofitFactory {
    ...
    private static final Interceptor cacheControlInterceptor = new Interceptor() {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (!CommNetWorkUtil.isNetworkConnected(CommBaseActivity.getAppContext())) {
    ...
    public static Retrofit getRetrofit() {
        synchronized (Object) {
            if (retrofit == null) {
                // 指定缓存路径,缓存大小 50Mb
                Cache cache = new Cache(new File(CommBaseActivity.getAppContext().getCacheDir(), "HttpCache"),
                        1024 * 1024 * 50);

                // Cookie 持久化
                ClearableCookieJar cookieJar =
                        new PersistentCookieJar(new SetCookieCache(), new SharedPrefsCookiePersistor(CommBaseActivity.getAppContext()));
