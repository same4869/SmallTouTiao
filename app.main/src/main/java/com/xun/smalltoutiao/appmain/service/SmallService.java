...
import com.xun.smalltoutiao.appmain.bean.PluginEntity;
import com.xun.smalltoutiao.appmain.bean.SmallEntity;
import com.xun.smalltoutiao.appmain.sp.SpMainManager;
import com.xun.smalltoutiao.libcomm.utils.FastJsonUtil;
...
//        mLocalBroadcastManager = LocalBroadcastManager.getInstance(this);
    ...
    private boolean smallCheckUpdate(String urlString) {
        ...
        try {
            ...
            SpMainManager.savePluginBundles(getApplicationContext(), plugin_bundles);
    ...
    private boolean smallUpdateBundles() {
        try {
            String plugin_bundles = SpMainManager.getPluginBundles(getApplicationContext());
            if (TextUtils.isEmpty(plugin_bundles)) {
                return false;
            }
            mSmallEntity = FastJsonUtil.parseJson(plugin_bundles, SmallEntity.class);
            ...
            boolean updateManifest = mSmallEntity.getManifest_code() > SpMainManager.getManifestCode(getApplicationContext());
            if (updateManifest) {
                // 更新注册表信息
            ...
            if (needUpdateOrAdd()) {
                ...
                if (updateManifest) {
                    SpMainManager.saveSmallAdd(getApplicationContext(), 1);
                    sendServiceStatus(STATUS_TOAST, "增加插件成功！");
                } else {
                    SpMainManager.saveSmallUpdate(getApplicationContext(), 1);
                    sendServiceStatus(STATUS_TOAST, "更新插件成功！");
                }
            }
            SpMainManager.saveManifestCode(getApplicationContext(), mSmallEntity.getManifest_code());
            SpMainManager.saveUpdatesCode(getApplicationContext(), mSmallEntity.getUpdates_code());
            SpMainManager.saveAdditionCode(getApplicationContext(), mSmallEntity.getAdditions_code());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return true;
    ...
    private boolean needUpdateOrAdd() {
        ...
        boolean hasUpdates = mSmallEntity.getUpdates_code() > SpMainManager.getUpdatesCode(getApplicationContext());
        boolean hasAdditions = mSmallEntity.getAdditions_code() > SpMainManager.getAdditionCode(getApplicationContext());
